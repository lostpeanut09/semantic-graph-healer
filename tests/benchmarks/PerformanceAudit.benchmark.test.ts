...
describe("Performance & Resource Audit Benchmarks", () => {
  describe("AjsonStorage Memory Footprint (Large Load)", () => {
    it("should handle reading 5000 lines of JSON without failure", async () => {
      ...
      console.log(
        `AjsonStorage ReadAll (5000 items) memory diff: ${memoryDiffMb.toFixed(2)} MB`,
      );
  ...
  describe("LLM Cost Reduction (Stage 0 Bypass Rate)", () => {
    it("should bypass LLM calls for unrelated content (Similarity < 0.4)", async () => {
      ...
      vi.mocked(requestUrl).mockResolvedValue({
        ...
        json: {
          choices: [
            { message: { content: "WINNER: A | SCORE: 90 | WHY: Match" } },
          ],
        },
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (
          result.includes("Status: REJECTED") &&
          result.includes("SEMANTIC_UNRELATED")
        ) {
          bypassCount++;
        }
      ...
      console.log(
        `LLM Stage 0 Bypass Rate: ${bypassCount}% (${bypassCount}/100)`,
      );

...
describe("GraphRagView", () => {
  ...
  it("executes query and displays answer", async () => {
    ...
    (
      pluginMock.graphRag as unknown as { query: ReturnType<typeof vi.fn> }
    ...
    await vi.waitFor(() => {
      expect(host.querySelector(".healer-rag-answer")?.textContent).toContain(
        "This is a test answer.",
      );
      expect(host.querySelector(".healer-rag-context")).toBeTruthy();
      expect(host.querySelector(".healer-comm-pill")?.textContent).toContain(
        "Cluster 1",
      );
    });

...
vi.mock("../../../src/views/components/ConfirmationModal", () => ({
  ConfirmationModal: vi
    .fn()
    .mockImplementation(function (app, suggestion, onConfirm) {
      return {
...
describe("DashboardStore", () => {
  ...
  const asPluginContext = (m: MockPlugin): PluginContext =>
    m as unknown as PluginContext;
  ...
  describe("Batch and Ignore Logic", () => {
    ...
    beforeEach(() => {
      ...
      (global as unknown as { Notice: MockFn }).Notice = vi
        .fn()
        .mockImplementation(() => {
          return {
            noticeEl: {
  ...
  describe("AI Logic Verification", () => {
    ...
    it("verifyAI calls validateBranching with context for branch suggestions", async () => {
      ...
      expect(
        mockPlugin.topology!.getContextForAIValidation,
    ...
    it("resolveChoice calls executor.resolveChoice", async () => {
      ...
      expect(mockPlugin.executor.resolveChoice!).toHaveBeenCalledWith(
        suggestion,
        "winner",
        ["loser"],
      );
    ...
    it("executeComplex triggers ConfirmationModal and calls executor.executeRelink", async () => {
      ...
      await vi.waitFor(() => {
        if (!store.fixedItems.has(suggestion.id))
          throw new Error("Not fixed yet");
      });

      expect(ConfirmationModal).toHaveBeenCalled();
      expect(mockPlugin.executor.executeRelink!).toHaveBeenCalledWith(
        suggestion,
      );
      expect(store.fixedItems.has(suggestion.id)).toBe(true);
    });

...
// Mock Obsidian
vi.mock("obsidian", () => {
  const MockSetting = vi.fn().mockImplementation(function (
    ...
    this.addDropdown = vi
      .fn()
      .mockImplementation((cb: (dropdown: Record<string, unknown>) => void) => {
        if (cb)
          cb({
    ...
    this.addText = vi
      .fn()
      .mockImplementation((cb: (text: Record<string, unknown>) => void) => {
        if (cb)
          cb({
    ...
    this.addButton = vi
      .fn()
      .mockImplementation((cb: (button: Record<string, unknown>) => void) => {
        if (cb)
          cb({
...
describe("EmbeddingSettings", () => {
  ...
  it("renders all setting fields", () => {
    ...
    expect(
      containerEl.querySelector(".healer-model-status")?.textContent,
    ).toContain("STABLE");
  });

  it("displays different status colors based on model health", () => {
    (
      mockCtx.plugin as unknown as { embedding: { modelStatus: string } }
    ).embedding.modelStatus = "MISALIGNED";
    renderEmbeddingSettings(containerEl, mockCtx);
    const statusEl = containerEl.querySelector(
      ".healer-model-status",
    ) as HTMLElement;
    ...
    (
      mockCtx.plugin as unknown as { embedding: { modelStatus: string } }
    ).embedding.modelStatus = "OFFLINE";
    renderEmbeddingSettings(containerEl, mockCtx);
    const statusEl2 = containerEl.querySelector(
      ".healer-model-status",
    ) as HTMLElement;

...
describe("NativeVaultAdapter", () => {
  ...
  it("should throw if getLinks is called before initialization", async () => {
    await expect(adapter.getLinks()).rejects.toThrow(
      "native-vault adapter: not initialized",
    );
  });
  ...
  it("should throw if getRichLinksForFile is called before initialization", async () => {
    ...
    await expect(adapter.getRichLinksForFile(mockFile)).rejects.toThrow(
      "native-vault adapter: not initialized",
    );
  });
  ...
  it("should normalize paths, filter self-links and non-markdown if configured", async () => {
    // Mock app.settings
    (
      app as unknown as { settings: { includeNonMarkdownHubs: boolean } }
    ).settings = { includeNonMarkdownHubs: false };

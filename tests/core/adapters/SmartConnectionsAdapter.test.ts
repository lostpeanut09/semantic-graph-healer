...
describe('SmartConnectionsAdapter', () => {
        ...
        mockApp = {
            ...
            metadataCache: {
                getFirstLinkpathDest: vi.fn((p: string) => {
                    if (p === 'folder/note.md' || p === 'folder/other.md' || p === 'folder/good.md') {
    ...
    describe('semanticQueryCache', () => {
        it('cache miss on first call: reads file', async () => {
            ...
            const query = await (
                adapter as unknown as {
                    buildSemanticQuery: (path: string) => Promise<string>;
                }
            ).buildSemanticQuery('folder/note.md');
        ...
        it('destroy() clears cache', async () => {
            ...
            const cacheSize = (adapter as unknown as { semanticQueryCache: { size: number } }).semanticQueryCache.size;
        ...
        it('returns path when file not found in vault', async () => {
            ...
            ).buildSemanticQuery('nonexistent/file.md');
    ...
    describe('queryAjsonFallback regression', () => {
        it('skips oversized files via stat bound (pre-read)', async () => {
            const mockVault = mockApp.vault as unknown as {
                adapter: {
        ...
        it('tries .ajson when .json exists but is empty', async () => {
            ...
            expect(mockVault.adapter.read).toHaveBeenCalledWith('.smart-env/smart_sources.json');
            expect(mockVault.adapter.read).toHaveBeenCalledWith('.smart-env/smart_sources.ajson');
            expect(result.length).toBeGreaterThan(0);
        });

        it('survives a single circular entry and continues scanning', async () => {
            const circular: { refs: string[]; self?: unknown } = {
                refs: ['folder/note.md'],
            };
        ...
        it('returns SearchResult[] objects without link field (Fix 3 typing separation)', async () => {
            ...
            for (const item of result) {
                ...
                expect(obj).not.toHaveProperty('link');
            }
        });

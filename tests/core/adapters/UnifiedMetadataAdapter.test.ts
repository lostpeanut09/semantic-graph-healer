...
const internals = (a: UnifiedMetadataAdapter): AdapterInternals =>
  a as unknown as AdapterInternals;
...
describe("UnifiedMetadataAdapter Hardening", () => {
  ...
  beforeEach(() => {
    ...
    adapter = new UnifiedMetadataAdapter(
      mockApp as unknown as ConstructorParameters<
        typeof UnifiedMetadataAdapter
      >[0],
      mockSettings as unknown as ConstructorParameters<
        typeof UnifiedMetadataAdapter
      >[1],
      {},
      {
  ...
  describe("Task 1: EventRef Cleanup", () => {
    it("should register metadataCache listener on initialize and unregister on destroy", async () => {
      ...
      const resolvedCall = mockApp.metadataCache.on.mock.calls.find(
        (call: unknown[]) => call[0] === "resolved",
      );
      expect(resolvedCall).toBeDefined();

      const resolvedCallIndex = mockApp.metadataCache.on.mock.calls.indexOf(
        resolvedCall as unknown[],
      );
      const eventRef =
        mockApp.metadataCache.on.mock.results[resolvedCallIndex].value;
  ...
  describe("Task 2: Parallel Link Aggregation and Deduplication", () => {
    it("aggregates links from multiple adapters in parallel", async () => {
      ...
      datacore.getLinksSafe = vi
        .fn()
        .mockResolvedValue([
          { sourcePath: "a", targetPath: "b", type: "link", confidence: 1 },
        ]);
      breadcrumbs.getLinksSafe = vi
        .fn()
        .mockResolvedValue([
          { sourcePath: "a", targetPath: "c", type: "link", confidence: 1 },
        ]);
      smartConnections.getLinksSafe = vi
        .fn()
        .mockResolvedValue([
          { sourcePath: "b", targetPath: "c", type: "link", confidence: 1 },
        ]);
      nativeVault.getLinksSafe = vi
        .fn()
        .mockResolvedValue([
          { sourcePath: "c", targetPath: "d", type: "link", confidence: 1 },
        ]);
  ...
  describe("destroy() lifecycle guard", () => {
    ...
    it("getRelatedNotes after destroy returns [] without invoking smartConnections", async () => {
      const sc = internals(adapter).smartConnections;
      sc.getRelatedNotes.mockResolvedValue([
        { path: "related.md", score: 0.9 },
      ]);

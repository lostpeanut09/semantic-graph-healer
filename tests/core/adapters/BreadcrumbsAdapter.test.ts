...
describe("BreadcrumbsAdapter", () => {
  ...
  const makeTFile = (path: string): TFile =>
    new (TFile as unknown as new (path: string) => TFile)(path) as TFile;

  const createMockApi = (
    overrides: Partial<MockBreadcrumbsApi> = {},
  ): MockBreadcrumbsApi => ({
    closedG: null,
    mainG: null,
  ...
  it("uses direct full path when file exists in vault", async () => {
    ...
    expect(getMatrixNeighbours).toHaveBeenCalledWith("test/note.md", [
      "up",
      "same",
      "down",
      "next",

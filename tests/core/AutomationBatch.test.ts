...
describe('AutomationBatch', () => {
    ...
    it('should correctly filter suggestions and execute batch repairs with UUID tagging', async () => {
        ...
        mockContext.executor.execute.mockImplementation(async (s: Suggestion) => {
    ...
    it('should rollback batch operations in reverse-chronological order and save cache', async () => {
        ...
        mockContext.executor.undo.mockImplementation(async (item: HistoryItem) => {
            undoOrder.push(item.action);
            return true;
        });
        ...
        expect(undoOrder).toEqual(['Fix 2', 'Fix 1']);

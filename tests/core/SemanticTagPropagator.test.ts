...
describe("SemanticTagPropagator", () => {
  ...
  beforeEach(() => {
    mockApp = {
      vault: {
        getAbstractFileByPath: vi.fn().mockImplementation((path: string) => {
          ...
          (f as unknown as { basename: string }).basename =
    ...
    propagator = new SemanticTagPropagator(
      mockApp,
      mockSettings,
      mockEngine,
      mockLlm,
  ...
  it("should suggest tag propagation when majority threshold is met", () => {
    ...
    (
      mockApp.metadataCache.getFileCache as unknown as ReturnType<typeof vi.fn>
  ...
  it("should respect the exclusion list", () => {
    ...
    (
      ...
      if (file.path === "P.md")
        return { tags: [{ tag: "#MOC" }, { tag: "#science" }] };
      if (file.path === "C1.md") return { tags: [{ tag: "#science" }] };

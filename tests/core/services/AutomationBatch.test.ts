...
describe("AutomationBatch", () => {
  ...
  it("should correctly filter suggestions and execute batch repairs with UUID tagging", async () => {
    ...
    (
      mockContext.executor.execute as unknown as ReturnType<typeof vi.fn>
    ).mockImplementation(async (s: Suggestion) => {
      expect(mockContext.executor.activeBatchId).toBeDefined();
      // The UUID generated by `generateId` has a prefix 'batch_' followed by a v4 UUID.
  ...
  it("should rollback batch operations in reverse-chronological order and save cache", async () => {
    ...
    const history1: HistoryItem = {
      ...
      batchId,
      mementoData: [
        { path: "file1.md", property: "prop", originalValue: "old1" },
      ],
    };
    ...
    const history2: HistoryItem = {
      ...
      batchId,
      mementoData: [
        { path: "file2.md", property: "prop", originalValue: "old2" },
      ],
    };
    ...
    (
      mockContext.executor.undo as unknown as ReturnType<typeof vi.fn>
    ).mockImplementation(async (item: HistoryItem) => {

...
describe("LinkPredictionEngine", () => {
  ...
  it("should synthesize suggestions from worker results without SmartConnections", async () => {
    (
      mockContext.settings as unknown as { enableSmartConnections: boolean }
    ...
    (
      mockContext.graphWorkerService.runAnalysis as unknown as ReturnType<
        typeof vi.fn
      >
    ).mockResolvedValue(workerResults);
    (
      mockContext.app.vault.getAbstractFileByPath as unknown as ReturnType<
        typeof vi.fn
      >

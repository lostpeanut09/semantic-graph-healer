...
describe("LlmService - AI Tribunal Logic", () => {
  ...
  beforeEach(() => {
    ...
    mockGetKey = vi.fn().mockResolvedValue("test-key") as unknown as (
      type: ApiKeyType,
    ) => Promise<string>;
  ...
  it("should NOT call secondary model if primary confidence >= safeZoneThreshold", async () => {
    vi.mocked(requestUrl).mockResolvedValueOnce({
      ...
      json: {
        choices: [
          { message: { content: "WINNER: A | SCORE: 85 | WHY: Good" } },
        ],
      },
  ...
  it("should call secondary model if primary confidence < safeZoneThreshold", async () => {
    ...
    vi.mocked(requestUrl).mockResolvedValueOnce({
      ...
      json: {
        choices: [
          { message: { content: "WINNER: A | SCORE: 70 | WHY: Okay" } },
        ],
      },
    } as unknown as RequestUrlResponse);
    ...
    vi.mocked(requestUrl).mockResolvedValueOnce({
      ...
      json: {
        choices: [
          { message: { content: "WINNER: A | SCORE: 90 | WHY: Better" } },
        ],
      },
    } as unknown as RequestUrlResponse);
  ...
  it("should result in CONFLICT when primary and secondary disagree", async () => {
    ...
    vi.mocked(requestUrl).mockResolvedValueOnce({
      ...
      json: {
        choices: [
          { message: { content: "WINNER: A | SCORE: 70 | WHY: A is good" } },
        ],
      },
    } as unknown as RequestUrlResponse);
    ...
    vi.mocked(requestUrl).mockResolvedValueOnce({
      ...
      json: {
        choices: [
          { message: { content: "WINNER: B | SCORE: 80 | WHY: B is better" } },
        ],
      },
    } as unknown as RequestUrlResponse);
  ...
  describe("validateTagInheritance", () => {
    it("should return true if LLM responds YES", async () => {
      ...
      const result = await service.validateTagInheritance(
        "Child",
        "tag",
        "Parent",
      );

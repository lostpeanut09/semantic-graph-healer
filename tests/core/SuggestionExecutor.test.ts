...
describe("SuggestionExecutor", () => {
  ...
  beforeEach(() => {
    ...
    executor = new SuggestionExecutor(
  ...
  describe("undo", () => {
    ...
    it("should restore frontmatter from mementoData", async () => {
      const historyItem: HistoryItem = {
        ...
        type: "fix",
        mementoData: [
          { path: "A.md", property: "up", originalValue: "[[OldParent]]" },
        ],
      };
      ...
      expect(mockContext.app.vault.getAbstractFileByPath).toHaveBeenCalledWith(
        "A.md",
      );
    ...
    it("should handle multiple memento entries", async () => {
      ...
      expect(result).toBe(true);
      expect(
        mockContext.app.fileManager.processFrontMatter,

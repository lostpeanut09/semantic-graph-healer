...
describe("Utils Hardening", () => {
  describe("HealerLogger (Ultra-Hardening Audit)", () => {
    ...
    it("ULTRA-3: handles log folder path collision gracefully", async () => {
      ...
      (
        plugin.app.vault.getAbstractFileByPath as unknown as ReturnType<
          typeof vi.fn
        >
    ...
    it("ULTRA-4: respects max log size and performs rotation", async () => {
      ...
      (
        plugin.app.vault.getAbstractFileByPath as unknown as ReturnType<
          typeof vi.fn
        >
      ).mockReturnValueOnce(mockFolder); // folder call
      (
        plugin.app.vault.getAbstractFileByPath as unknown as ReturnType<
          typeof vi.fn
        >
      ).mockReturnValueOnce(mockFile); // file call

      (
        plugin.app.vault.create as unknown as ReturnType<typeof vi.fn>
      ).mockResolvedValue(new TFile()); // New file after rotate

...
describe("HealerLogger Deduplication", () => {
  ...
  it("test_dedup_dropsIdenticalMessageWithinWindow", () => {
    ...
    const buffer = (
      logger as unknown as { logBuffer: Array<{ message: string }> }
    ).logBuffer;
  ...
  it("test_dedup_keepsDifferentModule", () => {
    ...
    const bufferB = (
      loggerB as unknown as { logBuffer: Array<{ message: string }> }
    ).logBuffer;
  ...
  it("test_dedup_keepsDifferentLevel", () => {
    ...
    expect(buffer.length).toBe(3);
    expect(buffer.map((e) => e.level).sort()).toEqual([
      "error",
      "info",
      "warn",
    ]);
  ...
  it("test_dedup_windowExpiry", () => {
    ...
    expect(
      (logger as unknown as { logBuffer: Array<unknown> }).logBuffer.length,
    ).toBe(1);
  ...
  it("test_dedup_prunesStaleFingerprints_keepsBoundedGrowth (CR-02 regression)", () => {
    ...
    const fingerprints = (
      logger as unknown as { recentFingerprints: Map<string, number> }
    ).recentFingerprints;
  ...
  it("test_dedup_dropsAbandonedModuleEntries (CR-02 regression)", () => {
    ...
    const moduleCounts = (
      logger as unknown as { moduleLogCounts: Map<string, number[]> }
    ).moduleLogCounts;
  ...
  describe("G5 — setDedupConfig coercion (Settings UI parity)", () => {
    it("coerces negative windowMs to 0 (disables dedup)", () => {
      logger.setDedupConfig({ windowMs: -100 });
      const cfg = (logger as unknown as { dedupConfig: { windowMs: number } })
        .dedupConfig;
    ...
    it("coerces negative perModuleCap to 0", () => {
      ...
      const cfg = (
        logger as unknown as { dedupConfig: { perModuleCap: number } }
      ).dedupConfig;
    ...
    it("coerces negative globalCap to 0", () => {
      ...
      const cfg = (logger as unknown as { dedupConfig: { globalCap: number } })
        .dedupConfig;
    ...
    it("coerces NaN windowMs to unchanged (Number.isFinite guard)", () => {
      const before = (
        logger as unknown as { dedupConfig: { windowMs: number } }
      ).dedupConfig.windowMs;
      logger.setDedupConfig({ windowMs: NaN });
      const after = (logger as unknown as { dedupConfig: { windowMs: number } })
        .dedupConfig.windowMs;
    ...
    it("coerces Infinity globalCap to unchanged (Number.isFinite guard)", () => {
      const before = (
        logger as unknown as { dedupConfig: { globalCap: number } }
      ).dedupConfig.globalCap;

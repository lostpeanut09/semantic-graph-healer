...
describe("HealerLogger Rate Limit", () => {
  ...
  it("test_perModuleCap_blocksAtCap", () => {
    ...
    const buffer = (logger as unknown as { logBuffer: Array<unknown> })
      .logBuffer;
  ...
  it("test_perModuleCap_doesNotBlockOtherModules", () => {
    ...
    const bufA = (loggerA as unknown as { logBuffer: Array<unknown> })
      .logBuffer;
    const bufB = (loggerB as unknown as { logBuffer: Array<unknown> })
      .logBuffer;
...
describe("G6 — Bolt rule stress test: pruning with 200+ entries", () => {
  ...
  it("prunes globalLogTimestamps correctly with 250 entries (write-pointer no-shift)", () => {
    ...
    const globalTimestamps = (
      logger as unknown as { globalLogTimestamps: number[] }
    ).globalLogTimestamps;
  ...
  it("prunes moduleLogCounts correctly after 200+ entries across modules", () => {
    ...
    const moduleCountsA = (
      loggerA as unknown as { moduleLogCounts: Map<string, number[]> }
    ).moduleLogCounts;
    const moduleCountsB = (
      loggerB as unknown as { moduleLogCounts: Map<string, number[]> }
    ).moduleLogCounts;
  ...
  it("pruneFingerprints handles 200+ entries correctly", () => {
    ...
    expect(fingerprints.has("info|FingerprintMod|fingerprint probe")).toBe(
      true,
    );
  ...
  it("buffer count is correct after pruning with large dataset", () => {
    ...
    globalTimestamps = (logger as unknown as { globalLogTimestamps: number[] })
      .globalLogTimestamps;

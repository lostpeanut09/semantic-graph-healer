...
1. **Does `registerCliHandler` support flag validation?**
    - **Answer:** No. The `flags` parameter in `registerCliHandler` provides type-casting for the pre-parsed `CliData`, but it does not perform semantic validation (e.g., range checks, enum constraints).
    - **Final Recommendation:** Use a Zod schema within the handler to validate `CliData` params (especially for `executeBatch` confidence thresholds) before passing them to the `AutomationApi`.

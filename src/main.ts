...
export default class SemanticGraphHealer extends Plugin {
  ...
  async onload() {
    ...
    this.engine = new UnifiedMetadataAdapter(
      this.app as ExtendedApp,
      this.settings,
      {
        datacore,
  ...
  private async initializeSecurity() {
    ...
    if (app.secretStorage || app.keychain) {
      this.logger.debug(
        "Obsidian secure storage detected. Verifying and migrating API keys...",
      );
  ...
  private registerViews() {
    this.registerView(
      DASHBOARD_VIEW_TYPE,
      (leaf) => new QuarantineDashboardView(leaf, this),
    );
  ...
  async loadSettings() {
    ...
    const baseSettings = Object.assign(
      ...
      DEFAULT_SETTINGS,
      loadedData,
    ) as SemanticGraphHealerSettings;

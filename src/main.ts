...
import type {
  AnalysisContext,
  ExecutionContext,
} from "./core/services/PluginContext";
...
import {
  GraphVisualizerView,
  GRAPH_VIEW_TYPE,
} from "./views/GraphVisualizerView";
...
export default class SemanticGraphHealer extends Plugin {
  ...
  async onload() {
    ...
    this.logger = new InstanceLogger(
      "SemanticGraphHealer",
      this,
      this.settings,
    );
    ...
    this.keychainService = new KeychainService({
      ...
      onCorruptionDetected: () => {
        new ResetKeychainModal(
          this.app as ExtendedApp,
          this.keychainService,
        ).open();
      },
    });
    this.graphWorkerService = new GraphWorkerService(
      this.logger,
      this,
    ...
    this.ladybugService = new LadybugService(
      this.app,
      this.manifest as unknown as ExtendedManifest,
    );
    ...
    const breadcrumbs = new BreadcrumbsAdapter(
      this.app,
      this.settings.logLevel === "debug",
    );
    ...
    this.topology = new TopologyAnalyzer(
      analysisContext,
      this.llm,
      this.engine,
    );
    ...
    this.tagPropagator = new SemanticTagPropagator(
      this.app,
      this.settings,
      this.engine,
      this.llm,
  ...
  async onExternalSettingsChange() {
    try {
      this.logger.info(
        "External settings change detected. Re-initializing engine...",
      );
      await this.loadSettings();
      (this.app as ExtendedApp).settings = this.settings;
      ...
      void this.refreshDashboard().catch((e) =>
        this.logger.error("Sync Refresh failed", e),
      );
    } catch (e) {
  ...
  private registerVaultEvents() {
    const triggerAnalysis = (file: TFile, force = false) => {
      ...
      if (!force && activeFile && activeFile.path === file.path) {
        this.logger.info(
          `Skipping analysis for active file: ${file.basename}. Will retry on focus change.`,
        );
        return;
      }
      ...
      const debounceTime = this.performanceService.isSafetyModeActive()
        ? 15000
        : 5000;
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        void (async () => {
          ...
          const isCanvas =
            file.extension === "canvas" && this.settings.includeNonMarkdownHubs;
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", () => {
        ...
        if (lastFile && (!currentFile || lastFile.path !== currentFile.path)) {
          this.logger.info(
            `Context switched from: ${lastFile.basename}. Triggering deferred analysis...`,
          );
    ...
    try {
      ...
      if (bridgeIssues.length > 0) {
        ...
        const newIssues = bridgeIssues.filter(
          (bi: Suggestion) =>
            !this.cache.suggestions.some((ps: Suggestion) => ps.id === bi.id),
        );
  ...
  private async initializeSecurity() {
    ...
    if (
      !this.settings.detectedModels ||
      this.settings.detectedModels.length === 0
    ) {
      this.settings.detectedModels = [...cloudFallbacks];
      settingsChanged = true;
    }
    if (
      !this.settings.secondaryDetectedModels ||
      this.settings.secondaryDetectedModels.length === 0
    ) {
    ...
    if (app.secretStorage || app.keychain) {
      ...
      if (this.settings.secondaryLlmApiKey) {
        await this.keychainService.setApiKey(
          "anthropic",
    ...
    this.registerObsidianProtocolHandler("healer-action", async (params) => {
      try {
        ...
        if (action === "scan") {
          ...
          new Notice(
            `Remote batch apply triggered (confidence >= ${confidence * 100}%)...`,
          );
          ...
          new Notice(
            `Batch undone: ${result.revertedCount} reverted, ${result.failedCount} fail.`,
          );
        }
      } catch (e) {
  ...
  private registerCliHandlers() {
    if (typeof this.registerCliHandler !== "function") {
      this.logger.info(
        "registerCliHandler is not supported by this version of Obsidian.",
      );
      return;
    }
    ...
    this.registerCliHandler(
      ...
      async (params) => {
        try {
          const silent =
            params.silent !== undefined ? params.silent === "true" : true;
  ...
  private registerViews() {
    ...
    this.registerView(
      GRAPH_VIEW_TYPE,
      (leaf) => new GraphVisualizerView(leaf, this),
    );
  }
  ...
  private registerCommands() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        try {
          ...
          const frontmatterLength = cache?.frontmatter
            ? Object.keys(cache.frontmatter).length
            : 0;
          ...
          const suggestion = await this.llm.callLlm(
            ragPrompt,
            this.settings.enableAiTribunal,
          );
          ...
          if (
            ...
            !suggestion.startsWith("CONFLICT")
          ) {
            const linkRegex = /\[\[(.*?)\]\]/g;
            let match;
            let foundAny = false;
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        try {
          ...
          const parentSuggestions = this.topology.deriveTagSuggestions(
            tags,
            activeFile.path,
          );
          ...
          const relevantSiblings = allSiblings.filter(
            (s: Suggestion) =>
              s.meta?.sourceNote === activeFile.basename ||
              s.meta?.targetNote === activeFile.basename,
          );
          ...
          if (combined.length > 0) {
            ...
            const newOnes = combined.filter(
              (s: Suggestion) =>
                !this.cache.suggestions.some((p: Suggestion) => p.id === s.id),
            );
            if (newOnes.length > 0) {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        for (const child of childNotes) {
          ...
          if (this.cache.suggestions.some((s: Suggestion) => s.id === stableId))
            continue;
        ...
        if (suggestions > 0) {
          ...
          void this.refreshDashboard();
          new Notice(
            `Lasso: ${suggestions} relationships proposed. Review in dashboard.`,
          );
        } else {
    ...
    this.addCommand({
      ...
      callback: async () => {
        try {
          const lastBatchItem = [...this.cache.history]
            .reverse()
            .find((item) => item.batchId);
  ...
  private pruneStaleSuggestions(newIssues: Suggestion[]): Suggestion[] {
    ...
    const persistentSuggestions = this.cache.suggestions.filter(
  ...
  public async analyzeGraph(silent = false): Promise<boolean> {
    ...
    try {
      const deterministicIssues =
        await this.topology.runDeterministicAnalysis();
      await sleep(10);
      ...
      if (this.settings.enableSemanticAudit) {
        semanticIssues = await this.topology.runSemanticChildValidation(
          this.currentAnalysisController?.signal,
        );
        await sleep(10);
      }
      ...
      const crossThematicIssues =
        await this.topology.runCrossThematicAnalysis();
      ...
      if (newTopologicalIssues.length > 0) {
        new Notice(
          `Scrutiny complete: ${newTopologicalIssues.length} issues detected.`,
        );
      } else {
  ...
  public async getApiKey(
    type: "openai" | "anthropic" | "deepseek" | "infranodus" | "custom",
  ): Promise<string> {
  ...
  async fetchInfraNodusGaps(): Promise<number> {
    ...
    try {
      this.logger.info(
        `Querying InfraNodus for structural gaps in ${activeFile.basename}...`,
      );
      const content = await this.app.vault.read(activeFile);
      ...
      const gaps: InfraGap[] =
        data?.graph?.graphologyGraph?.attributes?.gaps ?? data?.gaps ?? [];
  ...
  async loadSettings() {
    const loadedData =
      (await this.loadData()) as Partial<SemanticGraphHealerSettings>;
    ...
    if (baseSettings.hierarchies && Array.isArray(baseSettings.hierarchies)) {
      baseSettings.hierarchies = (
        baseSettings.hierarchies as Record<string, unknown>[]
      ).map((h) => ({
    ...
    if (!this.settings.hierarchies || this.settings.hierarchies.length === 0) {
      this.logger.warn(
        "No valid hierarchy found after load â€” restoring defaults.",
      );
    ...
    try {
      ...
      if (await this.app.vault.adapter.exists(bcPath)) {
        try {
          ...
          if (bcData?.edge_fields && Array.isArray(bcData.edge_fields)) {
            (bcData.edge_fields as { label: string; dir: string }[]).forEach(
              (ef) => {
                if (ef.dir === "up") h.up = [...new Set([...h.up, ef.label])];
                if (ef.dir === "down")
                  h.down = [...new Set([...h.down, ef.label])];
                if (ef.dir === "same")
                  h.same = [...new Set([...h.same, ef.label])];
                if (ef.dir === "next")
                  h.next = [...new Set([...h.next, ef.label])];
                if (ef.dir === "prev")
                  h.prev = [...new Set([...h.prev, ef.label])];
                if (ef.dir === "related") {
        ...
        try {
          ...
          if (ebOntology) {
            ...
            if (ebOntology.child && Array.isArray(ebOntology.child)) {
              h.down = [
                ...new Set([...h.down, ...safeArray(ebOntology.child)]),
              ];
            }
            if (ebOntology.friend && Array.isArray(ebOntology.friend)) {
              h.same = [
                ...new Set([...h.same, ...safeArray(ebOntology.friend)]),
              ];
            }
            found = true;
          }
          ...
          if (ebPropertyMap) {
            for (const [prop, direction] of Object.entries(ebPropertyMap)) {
              if (direction === "next")
                h.next = [...new Set([...h.next, prop])];
              if (direction === "prev")
                h.prev = [...new Set([...h.prev, prop])];
      ...
      if (found) {
        ...
        if (newProps.length > 0) {
          new Notice(
            `Imported ${newProps.length} new hierarchy properties from external plugins.`,
          );
        } else {

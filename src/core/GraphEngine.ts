...
export class GraphEngine {
  ...
  public buildGraph() {
    ...
    for (const [sourcePath, targets] of Object.entries(resolvedLinks)) {
      ...
      for (const [targetPath, rawCount] of Object.entries(targets)) {
        ...
        if (this.graph.hasEdge(sourcePath, targetPath)) {
          const prev = this.graph.getEdgeAttribute(
            sourcePath,
            targetPath,
            "weight",
          ) as number;
    ...
    HealerLogger.info(
      `Graph built: ${this.graph.order} nodes, ${this.graph.size} edges.`,
    );

    if (useGuardrails && (nodeCount >= maxNodes || edgeCount >= maxEdges)) {
      HealerLogger.warn(
        "Graph construction hit memory guardrails. Some nodes/edges were omitted.",
      );
    }
  }
  ...
  public async runPageRankAnalysis(): Promise<Suggestion[]> {
    HealerLogger.info(
      "Running Weighted PageRank (Log-Transformed) in background worker...",
    );

    // SOTA 2026: Proactive Fallback for fragmented graphs
    const isolatedNodes = this.graph
      .nodes()
      .filter((n) => this.graph.degree(n) === 0).length;
    ...
    try {
      ...
      const scores = await worker.runAnalysis<Record<string, number>>(
        "PAGERANK",
        nodes,
        edges,
        {
      ...
      return this.processScores(
        scores,
        "pagerank_auth",
        "PageRank authority (log-weighted)",
      );
    } catch (e) {
      HealerLogger.warn(
        "Background PageRank failed. Falling back to sync total degree.",
        e,
      );
      return this.runDegreeCentralityFallback();
    }
  ...
  private runDegreeCentralityFallback(): Suggestion[] {
    ...
    this.graph.forEachNode((node) => {
      ...
      this.graph.forEachEdge(node, (edge) => {
        totalWeight +=
          (this.graph.getEdgeAttribute(edge, "weight") as number) || 0;
      });
  ...
  public async runCommunityDetection(): Promise<Suggestion[]> {
    ...
    try {
      ...
      const worker = this.context.graphWorkerService;
      const communities = await worker.runAnalysis<Record<string, number>>(
        "COMMUNITY",
        nodes,
        edges,
        {
  ...
  public async runBetweennessAnalysis(): Promise<Suggestion[]> {
    HealerLogger.info(
      "Running Weighted Betweenness Centrality (Worker Bridges)...",
    );
    try {
      const nodes: Array<{ key: string; attributes: Record<string, unknown> }> =
        [];
      ...
      sorted.forEach(([path, score]) => {
        ...
        suggestions.push({
          ...
          meta: {
            description:
              "Key connectivity node bridging different clusters (weighted).",
            confidence: 85,
            sourceNote: file instanceof TFile ? file.basename : path,
          },
        });
   ...
   */
  public async runCoCitationAnalysis(
    minScore = 2,
    limit = 15,
  ): Promise<Suggestion[]> {
    ...
    try {
      ...
      >("COCITATION", nodes, edges, { minScore });
      ...
      HealerLogger.info(
        `Co-Citation: ${suggestions.length} implicit links discovered via worker.`,
      );
      return suggestions;
    } catch (e) {
      HealerLogger.error(
        "Co-citation analysis failed in worker, falling back to sync.",
        e,
      );
      return this.runCoCitationAnalysisSync(minScore, limit);
    }
   ...
   */
  public async runSimilarityAnalysis(options?: {
    limit?: number;
  }): Promise<Suggestion[]> {
    HealerLogger.info(
      "Running Deep Topology Similarity Analysis (Worker offloaded)...",
    );
  ...
  private runCoCitationAnalysisSync(
    minScore: number,
    limit: number,
  ): Suggestion[] {

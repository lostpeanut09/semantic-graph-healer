...
export class SemanticTagPropagator {
  ...
  public runTagPropagationAnalysis(): Suggestion[] {
    HealerLogger.info(
      "Starting Phase 3 AI Semantic Tag Propagation Analysis...",
    );
    const suggestions: Suggestion[] = [];

    // 1. Fetch all pages
    const query =
      this.settings.scanFolder && this.settings.scanFolder !== "/"
    ...
    if (pages.length > 5000) {
      HealerLogger.warn(
        `Tag propagation skipped: vault too large (${pages.length} nodes).`,
      );
      return [];
    }
    ...
    for (const page of pages) {
      ...
      const parentPaths = resolveLinkpathsToPaths(
        this.app,
        linkpaths,
        page.file.path,
        resolverCache,
    ...
    for (const [parentPath, children] of childrenByParent) {
      ...
      const parentTags =
        parentCache?.tags?.map((t) => t.tag.replace(/^#/, "")) || [];
      ...
      for (const parentTag of parentTags) {
        ...
        const childrenWithTag = children.filter((child) => {
          ...
          return childCache?.tags?.some(
            (t) => t.tag.replace(/^#/, "") === parentTag,
          );
        });
        ...
        if (coverageRatio > coverageThreshold && coverageRatio < 1.0) {
          for (const child of children) {
            const childCache = this.app.metadataCache.getFileCache(child);
            const hasTag = childCache?.tags?.some(
              (t) => t.tag.replace(/^#/, "") === parentTag,
            );
            ...
            if (!hasTag) {
              ...
              suggestions.push({
                id: generateId(
                  `tag_propagation_${parentPath}_${child.path}_${parentTag}`,
                ),

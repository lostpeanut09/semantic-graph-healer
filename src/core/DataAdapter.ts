...
export class SmartConnectionsAdapter {
  ...
  private getPluginInstance(): SmartConnectionsPlugin | null {
    ...
    const plugins = (this.app as unknown as { plugins: ObsidianPluginRegistry })
      .plugins;
  ...
  public async query(
    ...
    const smartSources =
      sc.main?.smart_sources ??
      sc.smart_sources ??
      sc.env?.smart_sources ??
      null;
    ...
    if (smartSources) {
      try {
        ...
        if (typeof ss.search === "function") {
          HealerLogger.info(
            `Smart Connections v4: Querying .search() for ${sourcePath}`,
          );
          results = await ss.search(sourcePath, { limit: limit + 1 });
        } else if (typeof ss.find === "function") {
          HealerLogger.info(
            `Smart Connections v3: Querying .find() for ${sourcePath}`,
          );
        ...
        const resultsArray = (Array.isArray(results) ? results : []) as Record<
          string,
          unknown
        >[];
        return resultsArray
          .filter((res) => {
            const rawPath =
              res.path ?? (res.item as Record<string, unknown>)?.path;
        ...
        HealerLogger.error(
          `Smart Connections API call failed (${errMsg}), falling back to AJSON index.`,
          e,
        );
      }
    }
  ...
  private async querySmartEnvFallback(
    sourcePath: string,
    limit: number,
  ): Promise<Suggestion[]> {
    ...
    try {
      ...
      const smartSourcesPath: string | undefined =
        cfg.smart_sources?.single_file_data_path;
      if (smartSourcesPath && (await adapter.exists(smartSourcesPath))) {
        ...
        if (sourcesRaw.includes(`"${sourcePath}"`)) {
          HealerLogger.info(
            "Smart Env fallback: structured correlation not available.",
          );
          return [];
        }
      }

...
export class HealerLogger {
  ...
  constructor(
    ...
    if (this.settings) {
      ...
      this.logFilePath =
        this.settings.logFilePath || "SemanticGraphHealer/logs";
  ...
  private shouldEmit(
    ...
    const overModuleCap =
      cfg.perModuleCap > 0 && moduleTimes.length >= cfg.perModuleCap;
    const overGlobalCap =
      cfg.globalCap > 0 && this.globalLogTimestamps.length >= cfg.globalCap;
  ...
  private async ensureLogFolder(): Promise<TFolder | null> {
    ...
    if (existing) {
      ...
      console.warn(
        `[HealerLogger] Path collision: "${folderPath}" is a file. Disabling file logging.`,
      );
      return null;
    }
  ...
  private async appendLogLine(file: TFile, line: string): Promise<void> {
    ...
    try {
      ...
      await vault.process(file, (existing) =>
        existing ? existing + "\n" + line : line,
      );
    } catch (e) {
  ...
  private async writeToFile(entry: LogEntry): Promise<void> {
    ...
    try {
      ...
      const abstractFile =
        this.plugin.app.vault.getAbstractFileByPath(fileName);
      let file: TFile;
  ...
  private addToBuffer(entry: LogEntry): void {
    ...
    const max = Number.isFinite(this.maxBufferSize)
      ? Math.floor(this.maxBufferSize)
      : 1000;

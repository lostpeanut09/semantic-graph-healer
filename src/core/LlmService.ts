...
export class LlmService {
  ...
  public async callLlm(
    ...
    HealerLogger.info(
      `AI Call initiated for model: ${this.settings.llmModelName}`,
    );
    ...
    const queryModel = async (
      ...
      const makeRequest = async (): Promise<{
        ...
        const normalizeEndpoint = (
          ...
          if (tgtPath === "responses" && base.endsWith("/v1/responses"))
            return base;
          if (
            tgtPath === "chat/completions" &&
      ...
      try {
        ...
        if (!this.validateLlmResponse(json, isResponsesApi)) {
          HealerLogger.warn(
            `Invalid LLM response structure: ${JSON.stringify(json).slice(0, 200)}`,
          );
          return "";
        }
        ...
        HealerLogger.error(
          `LLM error [${model}] (final attempt ${retryCount + 1}):`,
          e,
        );
    ...
    try {
      ...
      secondWinner =
        this.parseReasoningResult(secondResult).winner?.toLowerCase() || "";
    } catch (e) {
      HealerLogger.warn(

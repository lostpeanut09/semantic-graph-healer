  ...
  async initialize(): Promise<void> {
    ...
    this.initPromise = (async () => {
      try {
        ...
        const workerContent = await this.plugin.app.vault.adapter.read(
          `${pluginDir}/worker.js`,
        );
        ...
        this.logger.error(
          "Worker initialization failed. Plugin will gracefully degrade.",
          error,
        );
        this.worker = null;
        ...
        new Promise<T>((resolve, reject) => {
          ...
          const timeoutId = setTimeout(() => {
            ...
            if (callback) {
              ...
              callback.reject(
                new Error(
                  `Analysis timeout for ${type} after ${timeoutMs / 1000} seconds`,
                ),
              );
            }
          }, timeoutMs);

...
function runSimilarityAnalysis(graph: DirectedGraph, options: unknown, requestId: string, reporter?: ProgressReporter) {
    ...
    graph.forEachNode((source) => {
        ...
        candidates.forEach((target) => {
            ...
            // ⚡ Bolt Optimization: Avoid allocating intermediate Sets for intersection in tight loop
            // Calculate metrics on-the-fly to reduce memory pressure and GC pauses
            let sharedSize = 0;
            let adamicAdar = 0;
            let ra = 0;

    ...
    options: z
      .object({
        ...
        weights: z
          ...
          .optional(),
        fileStats: z
          .record(z.string(), z.object({ mtime: z.number() }))
          .optional(),
...
export const createProgressReporter = (
  postMessageFn: (msg: WorkerResponse) => void,
): ProgressReporter => ({
 ...
 */
export function handleGraphWorkerMessage(
  ...
  try {
    ...
    payload.edges.forEach((edge) => {
      if (!graph.hasNode(edge.source)) {
        if (policy === "strict")
          throw new Error(`Missing source node: ${edge.source}`);
        graph.addNode(edge.source, {});
      }
      if (!graph.hasNode(edge.target)) {
        if (policy === "strict")
          throw new Error(`Missing target node: ${edge.target}`);
    ...
    switch (type) {
      case "PAGERANK": {
        ...
        let pagerankOptions: PagerankOptions =
          (options as unknown as PagerankOptions) || {};
        if (safetyMode) {
          pagerankOptions = {
            ...pagerankOptions,
        ...
        result = pagerank(
          graph,
          pagerankOptions as Parameters<typeof pagerank>[1],
        );
        break;
      }
      ...
      case "TOPOLOGY_DIAGNOSTICS":
        validateGraphSize(
          "TOPOLOGY_DIAGNOSTICS",
          options,
          DEFAULT_LIMITS.TOPOLOGY_DIAGNOSTICS,
        );
      ...
      default:
        throw new Error(
          `Unsupported graph worker message type: ${String(type)}`,
        );
    }
...
function runSimilarityAnalysis(
  ...
  graph.forEachNode((source) => {
    ...
    if (processedNodes % 50 === 0 && reporter) {
      reporter.postProgress(
        requestId,
        processedNodes / nodeCount,
        `Analyzing similarity for ${source}...`,
      );
    }
    ...
    candidates.forEach((target) => {
      ...
      const unionSize =
        sourceNeighbors.size + targetNeighbors.size - shared.size;
 ...
 */
function runCoCitationAnalysis(
  graph: DirectedGraph,
  options: unknown,
  requestId: string,
  reporter?: ProgressReporter,
  ...
  graph.forEachNode((source) => {
    ...
    if (processedNodes % 50 === 0 && reporter) {
      reporter.postProgress(
        requestId,
        processedNodes / nodeCount,
        `Analyzing co-citation for ${source}...`,
      );
    }
...
function runTopologicalDiagnostics(
  ...
  graph.forEachNode((a) => {
    ...
    graph.outEdges(a).forEach((edgeAB) => {
      ...
      const typeAB = graph.getEdgeAttribute(edgeAB, "type") as
        | string
        | undefined;
      ...
      if (typeAB) {
        graph.outEdges(b).forEach((edgeBC) => {
          ...
          const typeBC = graph.getEdgeAttribute(edgeBC, "type") as
            | string
            | undefined;
    ...
    graph.outNeighbors(node).forEach((neighbor) => {
      if (!visited.has(neighbor)) {
        ...
        for (let i = 0; i < cyclePath.length; i++) {
          ...
          const tgt =
            i === cyclePath.length - 1 ? cyclePath[0] : cyclePath[i + 1];
          const edgeType = graph.getEdgeAttribute(src, tgt, "type") as
            | string
            | undefined;

...
export class QualityAnalyzer {
  ...
  public invalidateAliasCache(): void {
    ...
    HealerLogger.debug(
      "QualityAnalyzer: Alias cache invalidated due to vault change.",
    );
  }
  ...
  public async runQualityAnalysis(
    ...
    pages.forEach((page: DataviewPage, index: number) => {
      ...
      const resolved = extractResolvedPaths(
        this.app,
        page,
        allKeys,
        file.path,
      ...
      // 2. MOC Saturation
      const resolvedDown = extractResolvedPaths(
        this.app,
        page,
        hierarchy.down,
        file.path,
      ...
      hierarchyKeys.forEach((k) => {
        ...
        extractedTargets.forEach((target) => {
          ...
          let resolvedFile = this.app.metadataCache.getFirstLinkpathDest(
            key,
            file.path,
          );
          if (!resolvedFile) {
  ...
  public async querySmartConnections(
    sourcePath: string,
    limit: number,
  ): Promise<Suggestion[]> {

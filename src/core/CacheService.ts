...
export class CacheService {
  ...
  constructor(private plugin: Plugin) {
    ...
    const pluginDir =
      manifest.dir ??
      `.obsidian/plugins/${manifest.id ?? "semantic-graph-healer"}`;
    ...
    try {
      ...
      if (exists) {
        ...
        try {
          ...
          this._cache = {
            pendingSuggestions: Array.isArray(parsed.pendingSuggestions)
              ? parsed.pendingSuggestions
              : [],
          ...
          HealerLogger.warn(
            `CacheService: JSON corrupted. Preserving to ${corruptPath}`,
          );
        ...
        HealerLogger.info(
          "CacheService: Migrating suggestions/history from data.json to healer-cache.json...",
        );
        this._cache = {
      ...
      HealerLogger.error(
        "CacheService: Failed to load cache, starting fresh.",
        e,
      );
      this._cache = { pendingSuggestions: [], history: [] };
    }
  ...
  save(): void {
    ...
    this._saveTimer = setTimeout(() => {
      this.saveImmediate().catch((e) =>
        HealerLogger.error("CacheService: Debounced save failed.", e),
      );
    }, SAVE_DEBOUNCE_MS);
  }
    ...
    this._savePromise = this._savePromise
      .then(async () => {
        ...
        try {
          ...
          HealerLogger.warn(
            "CacheService: Direct rename failed, falling back to remove+rename.",
            e,
          );

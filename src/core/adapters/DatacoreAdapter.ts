...
function coerceToStartOfDay(v: unknown, fallbackMillis: number): unknown {
  if (isRecord(v)) {
    ...
    if (typeof fn === "function") {
      ...
      if (isRecord(result) && typeof result["toMillis"] === "function")
        return result;
    }
  }
  ...
  if (
    win.luxon?.DateTime &&
    typeof win.luxon.DateTime.fromMillis === "function"
  ) {
    const dt = win.luxon.DateTime.fromMillis(
      coerceToMillis(v) ?? fallbackMillis,
      { zone: "local" },
    );
 ...
 */
function coerceToDateTime(
  v: unknown,
  fallbackMillis: number,
  dv?: { date?: (v: string) => unknown } | null,
): unknown {
  if (v instanceof Date)
    return Number.isNaN(v.getTime()) ? new Date(fallbackMillis) : v;
  if (isRecord(v) && typeof v["toMillis"] === "function") return v;
  if (typeof v === "string") {
    const normalized = /^\d{8}$/.test(v)
...
/**
 * Bounded Map with FIFO eviction to prevent unbounded growth.
 */
...
export class DatacoreAdapter implements IMetadataAdapter {
  ...
  private pageChildrenCache = new BoundedMap<
    string,
  ...
  private parseDateStrict(
    ...
    if (isRecord(coerce) && typeof coerce["date"] === "function") {
      try {
        const parsed = Reflect.apply(
          coerce["date"] as (...args: unknown[]) => unknown,
          coerce,
          [normalized],
        );
        if (parsed != null) return parsed;

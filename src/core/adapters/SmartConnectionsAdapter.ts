      ...
      if (results.length > 0) {
        ...
        return results
          .map((res) => {
            ...
            const targetPath = this.normalizeNotePath(
              rawTarget,
              normalizedSource,
            );
  ...
  private async queryAjsonFallback(
    ...
    for (const singleFileFallback of singleFileFallbacks) {
      ...
      try {
        ...
        if (
          stat &&
          stat.size > SmartConnectionsAdapter.MAX_FALLBACK_FILE_SIZE
        ) {
          HealerLogger.warn(
            `SmartConnectionsAdapter: skipping oversized ${singleFileFallback} (${stat.size} bytes)`,
          );
          continue;
        }
    ...
    for (const envPath of envPaths) {
      ...
      try {
        ...
        const ajsonFiles = files.files
          ...
          .slice(0, 200); // Safety cap: avoid unbounded I/O on huge indexes

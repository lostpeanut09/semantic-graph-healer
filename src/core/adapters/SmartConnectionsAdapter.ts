  ...
  implements ISmartConnectionsPort
{
  ...
  public isAvailable(): boolean {
    ...
    const plugin = (this.app as ExtendedApp).plugins.getPlugin(
      "smart-connections",
    );
    return !!plugin;
  }
  ...
  private getPluginShape(): SmartConnectionsPluginShape | null {
    ...
    const raw = (this.app as ExtendedApp).plugins?.getPlugin?.(
      "smart-connections",
    );
    return raw && typeof raw === "object"
      ? (raw as unknown as SmartConnectionsPluginShape)
      : null;
  }
  ...
  private getGlobalSmartSources(): SmartSources | null {
    ...
    return (
      env.smart_sources ??
      env.env?.smart_sources ??
      env.main?.smart_sources ??
      null
    );
  }

  private resolveSmartSources(
    plugin: SmartConnectionsPluginShape | null,
  ): SmartSources | null {
  ...
  private resolveLegacyApi(
    plugin: SmartConnectionsPluginShape | null,
  ): LegacyScApi | null {
    return plugin?.api ?? plugin?.instance?.api ?? null;
  }
  public invalidate(path?: string): void {
    const normalized = this.normalizeInvalidatePath(path, path);
    if (!normalized) {
  ...
  protected override onDestroy(): void {
    this.semanticQueryCache.clear();
    this.logDebug("SmartConnectionsAdapter destroyed.");
  }
  ...
  private async runSmartSearch(
    ...
    const isReady = (() => {
      ...
      if (typeof gs.is_ready === "function")
        return (gs.is_ready as () => boolean)();
      return true;
    })();

    if (globalSources && !isReady) {
      HealerLogger.warn(
        "SmartConnectionsAdapter: environment not ready, skipping semantic search",
      );
    ...
    return this.firstNonEmpty(normalizedPath, [
      {
        ...
        run: () =>
          globalSources?.search
            ? Promise.resolve(
                globalSources.search(normalizedPath, { limit: searchLimit }),
              )
      ...
      },
      {
        ...
        run: () =>
          pluginSources?.search
            ? Promise.resolve(
                pluginSources.search(normalizedPath, { limit: searchLimit }),
              )
            : Promise.resolve(undefined),
      },
      {
      ...
      },
      {
        ...
        run: () =>
          legacyApi?.search
            ? Promise.resolve(
                legacyApi.search(semanticQuery, { limit: searchLimit }),
              )
            : Promise.resolve(undefined),
      },
      {
        ...
        run: () =>
          legacyApi?.find
            ? Promise.resolve(
                legacyApi.find({ query: semanticQuery, limit: searchLimit }),
              )
            : Promise.resolve(undefined),
      },
    ]);
  ...
  async getRelatedNotes(path: string, limit: number): Promise<RelatedNote[]> {
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        return results
          .map((res) => {
            ...
            if (
              !(
                this.app.vault.getAbstractFileByPath(targetPath) instanceof
                TFile
              )
            )
              return null;
      ...
      HealerLogger.error(
        "SmartConnectionsAdapter: search pipeline failed for " +
          normalizedSource,
        e,
      );
    }
    ...
    const fallbackResults = await this.queryAjsonFallback(
      normalizedSource,
      limit,
    );
    const seen = new Set<string>();
   ...
   */
  private async queryAjsonFallback(
    sourcePath: string,
    limit: number,
  ): Promise<SearchResult[]> {
    ...
    for (const singleFileFallback of singleFileFallbacks) {
      ...
      try {
        ...
        if (
          statSize !== null &&
          statSize > SmartConnectionsAdapter.MAX_FALLBACK_FILE_SIZE
        ) {
          HealerLogger.warn(
        ...
        for (const [targetKey, targetVal] of Object.entries(
        ...
        HealerLogger.error(
          "SmartConnectionsAdapter: failed reading " + singleFileFallback,
          e,
        );
      }
    }
    ...
    for (const envPath of envPaths) {
      ...
      try {
        ...
        for (const f of ajsonFiles) {
          ...
          try {
            try {
              ...
              if (
                fstat &&
                fstat.size > SmartConnectionsAdapter.MAX_FALLBACK_FILE_SIZE
              ) {
                HealerLogger.warn(
        ...
        HealerLogger.error(
          "SmartConnectionsAdapter: AJSON fallback failed for dir " + envPath,
          e,
        );
      }
    }

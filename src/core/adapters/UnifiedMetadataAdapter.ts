...
export class UnifiedMetadataAdapter implements IMetadataAdapter {
  ...
  constructor(
    ...
    this.breadcrumbs =
      dependencies.breadcrumbs ?? new BreadcrumbsAdapter(this.app, debug);
    this.smartConnections =
      dependencies.smartConnections ??
      new SmartConnectionsAdapter(this.app, debug);
    this.nativeVault =
      dependencies.nativeVault ?? new NativeVaultAdapter(this.app, debug);

    this.pageCache = new StructuralCache<DataviewPage | null>(
      this.app,
      options,
    );
    this.hierarchyCache = new StructuralCache<HierarchyNode | null>(
      this.app,
      options,
    );
  ...
  public async initialize(): Promise<void> {
    ...
    for (const { name, adapter } of adapters) {
      if (!adapter.isAvailable()) {
        new Notice(
          `Semantic Graph Healer: ${name} is not available. Some semantic links may be missing.`,
        );
    ...
    this.app.metadataCache.on("resolved", () => {
      HealerLogger.debug(
        "UnifiedMetadataAdapter: metadataCache resolved, triggering debounced refresh",
      );
  ...
  private async withCoalescing<T>(
    key: string,
    factory: () => Promise<T>,
  ): Promise<T> {
  ...
  getPage(path: string): DataviewPage | null {
    ...
    const page = this.safeExecute(
      () => this.datacore.getPage(key),
      null,
      `getPage(${key})`,
    );
    ...
    const pagePath =
      (page as { path?: string; $path?: string }).path ||
      (page as { path?: string; $path?: string }).$path;
    if (pagePath && pagePath !== key) {
      HealerLogger.debug(
        `UnifiedMetadataAdapter: path mismatch for ${key} (got ${pagePath})`,
      );
    }
  ...
  async queryPages(query: string): Promise<DataviewPage[]> {
    return this.safeExecuteAsync(
      () => this.datacore.queryPages(query),
      [],
      `queryPages(${query})`,
    );
  }
  ...
  getBacklinks(path: string): string[] {
    return this.safeExecute(
      () => this.datacore.getBacklinks(path),
      [],
      `getBacklinks(${path})`,
    );
  }
  ...
  async getHierarchy(path: string): Promise<HierarchyNode | null> {
    if (this._isDestroyed) {
      HealerLogger.debug(
        `getHierarchy(${path}) called after destroy — skipped`,
      );
      return null;
    }
    const key = this.normalizeCacheKey(path, path);
  ...
  async getRelatedNotes(path: string, limit: number): Promise<RelatedNote[]> {
    if (this._isDestroyed) {
      HealerLogger.debug(
        `getRelatedNotes(${path}) called after destroy — skipped`,
      );
      return [];
    }
  ...
  public destroy(): void {
    ...
    const destroyCache = (
      ...
      try {
        ...
        HealerLogger.error(
          `[UnifiedMetadataAdapter] cache.destroy failed (${name}):`,
          e as Error,
        );
      }
    };

...
export class TopologyAnalyzer {
  ...
  private getScanQuery(): string {
    return this.context.settings.scanFolder &&
  ...
  public async runDeterministicAnalysis(): Promise<Suggestion[]> {
    HealerLogger.info(
      "Starting deterministic graph scrutiny (Precision Path mode)...",
    );
    ...
    for (const page of pages) {
      (["up", "down", "next", "prev", "same", "related"] as const).forEach(
        (relType) => {
          ...
          resolvedPaths.forEach((p) =>
            relMaps[relType].get(page.file.path)!.add(p),
          );
        },
      );
      ...
      if (count % this.BATCH_SIZE === 0 && this.YIELD_INTERVAL > 0) {
        await new Promise((resolve) =>
          setTimeout(resolve, this.YIELD_INTERVAL),
        );
      }
    }
    ...
    const checkArray = this.context.settings.requireRelatedReciprocity
      ? allTypes
      : reciprocalTypes;
    ...
    for (const pageA of pages) {
      ...
      checkArray.forEach((relType) => {
        ...
        const invRelType =
          relType === "related" ? "related" : inverseMap[relType];
        ...
        targetPaths.forEach((pathB) => {
          ...
          if (relType !== "next" && relType !== "prev") {
            ...
            if (!bTargetsBack.has(pathA)) {
              ...
              const severity =
                this.context.settings.strictDownCheck && relType === "down"
                  ? "error"
                  : "suggestion";
              const targetBasename =
                pathB.split("/").pop()?.replace(".md", "") || pathB;
              suggestions.push({
                ...
                meta: {
                  property: invRelType,
                  propertyKey:
                    (hierarchy as Record<string, string[]>)[invRelType]?.[0] ||
                    invRelType,
          ...
          if (
            (relType === "next" || relType === "prev") &&
            !this.isBreadcrumbsActive()
          ) {
    ...
    if (this.context.settings.enableDeepTopology) {
      HealerLogger.info(
        "Engaging Deep Topology Discovery (Worker-offloaded)...",
      );
  ...
  public async runIncongruenceAnalysis(): Promise<Suggestion[]> {
    HealerLogger.info(
      "Starting incongruence analysis (Precision Path mode)...",
    );
    ...
    this.context.settings.customTopologyRules.forEach((r) => {
      try {
        ...
        HealerLogger.error(
          `Invalid RegExp pattern in settings: "${r.pattern}"`,
          e,
        );
      }
    });
    ...
    for (const page of pages) {
      ...
      for (const dir of directionalTypes) {
        if (dir.type === "down" && !this.context.settings.strictDownCheck)
          continue;

        const linkpaths = extractLinkpaths(page, dir.keys);
        const uniquePaths = resolveLinkpathsToPaths(
          this.context.app,
          linkpaths,
          file.path,
          resolverCache,
        ...
        if (dir.type === "up") {
          ...
          dir.type === "next" &&
          !this.context.settings.allowNextBranching
        ) {
          maxThreshold = 1;
        } else if (
          dir.type === "prev" &&
          !this.context.settings.allowPrevBranching
        ) {
          maxThreshold = 1;
        ...
        for (const key of dir.keys) {
          const customRule = compiledRules.find(
            (r) => r.regex.test(file.path) && r.property === key,
          );
          if (customRule)
        ...
        if (uniquePaths.length > maxThreshold) {
          const isVerifiableBranching =
            (dir.type === "next" || dir.type === "prev") &&
            this.context.settings.requireAIBranchValidation;

          const competingSorted = uniquePaths.slice().sort();
          const competitorLinks = competingSorted.map((p) =>
            pathToWikilink(this.context.app, p, file.path),
          );
  ...
  public deriveTagSuggestions(tags: string[], filePath: string): Suggestion[] {
    ...
    for (const tag of tags) {
      ...
      if (parts.length > 1) {
        ...
        const parentFile = this.context.app.metadataCache.getFirstLinkpathDest(
          parentCandidate,
          filePath,
        );
        ...
        if (parts.length > 2) {
          ...
          const rootFile = this.context.app.metadataCache.getFirstLinkpathDest(
            rootCandidate,
            filePath,
          );
  ...
  public async runStructuralGapAnalysis(file: TFile): Promise<Suggestion[]> {
    ...
    const scSuggestions =
      this.context.settings.enableSmartConnections && adapter
        ? await adapter.query(
            file.path,
            this.context.settings.smartConnectionsLimit,
          )
        : [];
  ...
  public deriveTagSiblings(): Suggestion[] {
    HealerLogger.info(
      "Starting tag sibling detection (Precision Path mode)...",
    );
    const suggestions: Suggestion[] = [];
    const hierarchy = this.context.settings.hierarchies?.[0];
    if (!hierarchy) return [];
    ...
    pages.forEach((page: DataviewPage) => {
      ...
      const tagArray = Array.isArray(tags)
        ? tags
        : Array.from(tags as Iterable<string>);
  ...
  public async runBridgeScrutiny(scopeFile?: TFile): Promise<Suggestion[]> {
    ...
    const bridgeLimit = this.context.settings.enableDeepGraphAnalysis
      ? 1500
      : 2500;
    ...
    pages.forEach((p: DataviewPage) => {
      (["up", "down", "next", "prev"] as const).forEach((rel) => {
        ...
        const resolved = resolveLinkpathsToPaths(
          this.context.app,
          linkpaths,
          p.file.path,
          resolverCache,
    ...
    if (maxDepth === 0) {
      HealerLogger.info(
        "Bridge Scrutiny disabled (bridgeScrutinyMaxDepth = 0).",
      );
      return [];
    }
  ...
  public async runCycleAnalysis(): Promise<Suggestion[]> {
    await Promise.resolve();
    HealerLogger.info(
      "Starting hierarchical cycle detection (Precision Path mode)...",
    );
    const suggestions: Suggestion[] = [];
    const hierarchy = this.context.settings.hierarchies?.[0];
    if (!hierarchy) return [];
  ...
  public async runFlowStagnationAnalysis(): Promise<Suggestion[]> {
    await Promise.resolve();
    HealerLogger.info(
      "Starting flow stagnation analysis (Precision Path mode)...",
    );
    const suggestions: Suggestion[] = [];
    ...
    fileMap.forEach((file, path) => {
      ...
      const isExcalidraw =
        file.name.endsWith(".excalidraw.md") || file.extension === "excalidraw";
  ...
  public async getContextForAIValidation(
    ...
    for (const targetPath of targetPaths) {
      ...
      const targetFile =
        this.context.app.vault.getAbstractFileByPath(targetPath);
      const content =
   ...
   */
  public async runSemanticChildValidation(
    signal?: AbortSignal,
  ): Promise<Suggestion[]> {
    ...
    HealerLogger.info(
      "Starting semantic child validation (AI-powered deep audit)...",
    );
    const suggestions: Suggestion[] = [];
    ...
    if (pages.length > 3000) {
      HealerLogger.warn(
        `Semantic child validation skipped: vault too large (${pages.length} nodes).`,
      );
      return [];
    }
    ...
    const downKeys =
      hierarchy.down.length > 0 ? hierarchy.down : ["down", "child"];

    // Build parent-child map
    const parentChildMap = new Map<
      string,
      Array<{ childPath: string; childName: string; mtime: number }>
    >();
    const resolverCache = new Map<string, string | null>();
    ...
    for (const page of pages) {
      ...
      const childPaths = resolveLinkpathsToPaths(
        this.context.app,
        linkpaths,
        page.file.path,
        resolverCache,
      ...
      for (const childPath of childPaths) {
        ...
        const childFile =
          this.context.app.vault.getAbstractFileByPath(childPath);
    ...
    for (const [parentPath, children] of parentChildMap.entries()) {
      ...
      const parentFile =
        this.context.app.vault.getAbstractFileByPath(parentPath);
      if (!(parentFile instanceof TFile)) continue;

      const parentContent = await this.context.app.vault
        .read(parentFile)
        .catch(() => "");

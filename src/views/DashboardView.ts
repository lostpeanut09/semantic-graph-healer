...
export class QuarantineDashboardView extends ItemView {
  ...
  public async refresh() {
    ...
    const mainWrapper = contentEl.createDiv({
      cls: "healer-dashboard-container",
    });
    ...
    this.listContainer = mainWrapper.createDiv({
      cls: "healer-dashboard-list-area",
    });
    this.renderList();
  }
  ...
  private renderFrame(container: HTMLElement) {
    ...
    const bannerPath = this.plugin.app.vault.adapter.getResourcePath(
      `${dir}/assets/banner.png`,
    );
    ...
    const headerRow = container.createDiv({
      cls: "healer-dashboard-header-row",
    });
    ...
    // --- FILTER DROPDOWN ---
    const filterContainer = headerRow.createDiv({
      cls: "healer-filter-container",
    });
  ...
  private renderList() {
    ...
    try {
      ...
      const renderCategory = (
        ...
        const items = displaySuggestions.filter(
          (s) => (s.category || "suggestion") === category,
        );
        if (items.length === 0) return;

        const sectionWrapper = this.listContainer.createDiv({
          cls: "healer-dashboard-section",
        });
        sectionWrapper.addClass(`healer-category-${category}`);

        new Setting(sectionWrapper)
          .setName(title)
          .setHeading()
          .setDesc(subtitle);
      ...
      if (displaySuggestions.length > 0) {
        ...
        renderCategory(
          "suggestion",
          "Suggestions",
          "Proposed links and structural improvements.",
        );
        renderCategory(
          "info",
          "Info",
          "Minor quality observations and architectural notes.",
        );
      } else {
  ...
  private renderSuggestionCard(parent: HTMLElement, suggestion: Suggestion) {
    ...
    const card = parent.createDiv({
      cls: `healer-suggestion-card healer-${category}-card`,
    });
    ...
    if (category !== "info") {
      const btnExec = btnDiv.createEl("button", {
        text: "Execute",
        cls: "healer-btn-execute",
      });
      btnExec.onclick = () => {
        if (suggestion.id.startsWith("bridge_gap")) {
          this.handleExecuteRelink(suggestion).catch((e) =>
            HealerLogger.error("Relink failed", e),
          );
        } else {
          this.handleExecute(suggestion).catch((e) =>
            HealerLogger.error("Execute failed", e),
          );
        }
      };
    }
    ...
    const isVerifiableBranching =
      ...
      (suggestion.meta?.property === "next" ||
        suggestion.meta?.property === "prev");
    const isVerifiableTag =
      suggestion.type === "semantic" && suggestion.meta?.property === "tags";
    ...
    if (isVerifiableBranching || isVerifiableTag) {
      ...
      const getContextWithTimeout = async (
        ...
        return Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(
              () => reject(new Error("Context gathering timeout (10s)")),
              timeoutMs,
            ),
          ),
        ]);
      ...
      const topologyInterface = this.plugin
        .topology as unknown as ValidationTopology;
      ...
      btnAiVerify.onclick = async () => {
        ...
        try {
          ...
          if (isVerifiableBranching) {
            ...
            if (
              typeof topologyInterface.getContextForAIValidation === "function"
            ) {
          ...
          if (e instanceof Error && e.message.includes("timeout")) {
            new Notice(
              "Context gathering timed out. File may be locked or too large.",
            );
      ...
      btnReason.onclick = () => {
        this.handleReasoning(suggestion).catch((e) =>
          HealerLogger.error("Reasoning failed", e),
        );
      };
    }
    ...
    const btnIgnore = btnDiv.createEl("button", {
      text: "Ignore",
      cls: "healer-btn-ignore",
    });
    btnIgnore.onclick = () => {
  ...
  private renderChoiceUI(card: HTMLElement, suggestion: Suggestion) {
    const values =
      suggestion.meta?.competingValues ?? suggestion.meta?.losers ?? [];
    ...
    const whyLink = card.createDiv({
      ...
      cls: "healer-reasoning-link",
    });
    whyLink.onclick = () => {
      this.showReasoningSidebar(suggestion).catch((e) =>
        HealerLogger.error("Show sidebar failed", e),
      );
    };
  }
  ...
  private async handleResolveChoice(
    suggestion: Suggestion,
    winner: string,
    losers: string[],
  ) {
    const success = await this.plugin.executor.resolveChoice(
      suggestion,
      winner,
      losers,
    );

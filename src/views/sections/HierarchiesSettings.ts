...
export function renderHierarchiesSettings(containerEl: HTMLElement, ctx: SectionContext) {
    const { plugin } = ctx;
    ...
    if (h) {
        ...
        customRulesSetting.addTextArea((text) => {
            text.setValue(JSON.stringify(plugin.settings.customTopologyRules, null, 2));
            text.inputEl.rows = 6;

...
export function renderResilienceSettings(containerEl: HTMLElement, ctx: SectionContext) {
    const { plugin } = ctx;
    ...
    new Setting(containerEl)
        ...
        .addText((text) =>
            text.setValue(plugin.settings.llmRetryableStatuses.join(', ')).onChange((value) => {

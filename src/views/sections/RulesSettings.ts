...
export function renderRulesSettings(containerEl: HTMLElement, ctx: SectionContext) {
    const { plugin } = ctx;
    ...
    new Setting(containerEl)
        ...
        .setDesc('Assume that if node a links to node b, node b should link back to node a with the inverse relation.')
        .addToggle((toggle) =>
            toggle.setValue(plugin.settings.impliedSymmetricEdges).onChange(async (v) => {
    ...
    new Setting(containerEl)
        ...
        .addToggle((toggle) =>
            toggle.setValue(plugin.settings.impliedTransitiveSiblings).onChange((value) => {
    ...
    new Setting(containerEl)
        ...
        .addToggle((toggle) =>
            toggle.setValue(plugin.settings.detectTaxonomicSkips).onChange((value) => {

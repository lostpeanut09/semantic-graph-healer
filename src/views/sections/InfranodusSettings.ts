...
export function renderInfranodusSettings(
  ...
  createHeader(
    "Network synergy",
    "Advanced science and structural gap analysis.",
  );
  ...
  new Setting(containerEl)
    .setName("Token")
    .setDesc(
      "Enter your secure token for the infranodus network analysis service.",
    )
    .addText((text) => {
      text
        .setPlaceholder("Enter infranodus token...")
        .setValue(plugin.settings.infraNodusApiKey);
  ...
  new Setting(containerEl)
    ...
    .addButton((btn) =>
      btn.setButtonText("Fetch gaps").onClick(async () => {
        ...
        if (count > 0) {
          new Notice(
            `System identified ${count} structural gaps! Review actions.`,
          );
        } else if (count === 0) {

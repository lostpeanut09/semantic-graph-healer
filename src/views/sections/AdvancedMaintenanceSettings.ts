...
export function renderAdvancedMaintenanceSettings(containerEl: HTMLElement, ctx: SectionContext) {
    ...
    new Setting(containerEl)
        ...
        .addButton((button) => {
            button.setButtonText('Export').onClick(async () => {
                const settingsExport = { ...plugin.settings } as Record<string, unknown>;
                ...
                input.onchange = async (e: Event) => {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            plugin.logger.error('Schema validation failed during import', result.error);
                        }
                    } catch (err: unknown) {
    ...
    new Setting(containerEl)
        ...
        .addToggle((toggle) =>
            toggle.setValue(plugin.settings.enableGraphGuardrails).onChange((value) => {

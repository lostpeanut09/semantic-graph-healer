...
export function renderLoggingSettings(
  ...
  createHeader(
    "Logging and debug",
    "Fine-grained control over plugin diagnostics.",
  );
  ...
  new Setting(containerEl)
    ...
    .addToggle((toggle) =>
      toggle
        .setValue(plugin.settings.enableFileLogging)
  ...
  createHeader(
    ...
    "Prevent log spam from rapid-fire or repeated errors.",
  );
  ...
  new Setting(containerEl)
    ...
    .setDesc(
      "Time window during which identical (level + module + message) log calls are collapsed into one.",
    )
    .addText((text) => {
      text
        .setPlaceholder("5000")
  ...
  new Setting(containerEl)
    ...
    .setDesc(
      "Upper bound on log calls accepted from a single module within the window.",
    )
    .addText((text) => {
      text
        .setPlaceholder("10")
  ...
  new Setting(containerEl)
    ...
    .setDesc(
      "Upper bound on log calls accepted from all modules within the window.",
    )
    .addText((text) => {
      text
        .setPlaceholder("100")

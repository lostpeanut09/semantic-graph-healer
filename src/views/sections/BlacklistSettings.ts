...
export function renderBlacklistSettings(containerEl: HTMLElement, ctx: SectionContext) {
    ...
    new Setting(containerEl)
        ...
        .setDesc(`You have ${plugin.settings.proximityIgnoreList.length} items currently ignored.`)
        .addButton((btn) =>
            btn
    ...
    plugin.settings.proximityIgnoreList.slice(-10).forEach((link: string) => {
        const s = new Setting(containerEl).setName(link).addButton((btn) =>
            btn
                ...
                .onClick(async () => {
                    plugin.settings.proximityIgnoreList = plugin.settings.proximityIgnoreList.filter(
                        (l: string) => l !== link,
                    );
                    await plugin.saveSettings();
                    refresh();

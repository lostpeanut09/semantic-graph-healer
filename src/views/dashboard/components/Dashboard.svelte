...
<div class="healer-dashboard-container">
  ...
  <nav class="healer-tabs" aria-label="Dashboard views" style="display: flex; gap: 8px; margin-bottom: 1em; border-bottom: 1px solid var(--background-modifier-border); padding-bottom: 8px; overflow-x: auto;">
    {#each tabs as tab}
      <button
  ...
  <div class="healer-dashboard-list-area" id="panel-{activeTab}">
        ...
        <div class="healer-history-row" style="display: flex; gap: 8px; font-size: 0.9em; margin-bottom: 4px; align-items: center;">
          ...
          {#if item.mementoData && item.mementoData.length > 0}
            <button
              ...
              aria-label="Undo {item.action} on {item.file}"

...
interface ForceGraphInstance {
  ...
  onNodeClick(
    fn: (node: ForceGraphNode, event: MouseEvent) => void,
  ): ForceGraphInstance;
  onLinkClick(
    fn: (link: ForceGraphLink, event: MouseEvent) => void,
  ): ForceGraphInstance;
  graphData(data: {
...
export class GraphVisualizerView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    this.graph = (
      ForceGraph3D as unknown as (el: HTMLElement) => ForceGraphInstance
    )(container)
      ...
      .nodeColor((node: ForceGraphNode) => {
        if (node && node.isCycle) {
          const pulse =
            Math.sin((Date.now() - this.startTime) / 200) * 0.5 + 0.5;
      ...
      .onNodeClick((node: ForceGraphNode, event: MouseEvent) => {
        ...
        suggestions.sort(
          (a, b) =>
            (severityMap[a.category] ?? 3) - (severityMap[b.category] ?? 3),
        );
      ...
      .onLinkClick((link: ForceGraphLink, event: MouseEvent) => {
        ...
        const linkSourceNode =
          typeof link.source === "object" ? link.source : null;
        const linkTargetNode =
        ...
        const suggestion = this.plugin.cache.suggestions.find(
          (s) =>
            (s.meta?.sourcePath === sourceId &&
              s.meta?.targetPath === targetId) ||
            (s.meta?.sourcePath === targetId &&
              s.meta?.targetPath === sourceId),
        );
  ...
  async refresh(): Promise<void> {
    ...
    if (this.plugin.performanceService.isSafetyModeActive()) {
      this.plugin.logger.info(
        "Safety Mode Active: Pausing simulation after initial layout.",
      );
      setTimeout(() => {
  ...
  onClose(): Promise<void> {
    ...
    if (this.graph) {
      this.plugin.logger.info(
        "Destroying graph visualization and cleaning up WebGL context...",
      );

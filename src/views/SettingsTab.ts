...
export class SemanticHealerSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    const bannerPath = this.app.vault.adapter.getResourcePath(
    ...
    const ctx: SectionContext = {
      ...
      runModelDetection: (button, isPrimary) =>
        this.runModelDetection(button, isPrimary),
    };
  ...
  async runModelDetection(button: ButtonComponent, isPrimary: boolean) {
    const endpoint = isPrimary
      ...
      : this.plugin.settings.secondaryLlmEndpoint;
    const apiKey = isPrimary
      ? await this.plugin.getApiKey("openai")
      : await this.plugin.getApiKey("anthropic");
    ...
    try {
      ...
      new Notice(
        `Testing ${isPrimary ? "primary" : "secondary"} connection...`,
      );

...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-empty-state" style="padding: 3em; text-align: center; border: 2px dashed var(--background-modifier-border); border-radius: 8px; color: var(--text-muted); margin-top: 1em;">
      <div style="font-size: 3em; margin-bottom: 0.2em;">🔍</div>
      <p style="margin: 0; font-size: 1.1em;">Ask a global question about your vault to explore insights from your knowledge graph.</p>
    </div>
  {/if}
</div>

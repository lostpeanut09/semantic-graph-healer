...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-card healer-empty-state" style="padding: 2em; text-align: center; border-style: dashed; border-color: var(--background-modifier-border); border-width: 2px; border-radius: 8px; color: var(--text-muted);">
      <span aria-hidden="true" style="font-size: 2em; display: block; margin-bottom: 0.5em;">🔍</span>
      <p>Ask a question to explore insights from your knowledge graph.</p>
    </div>
  {/if}

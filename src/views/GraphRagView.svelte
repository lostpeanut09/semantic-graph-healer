...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-empty-state" style="border: 2px dashed var(--background-modifier-border); padding: 2em; text-align: center; border-radius: 8px; color: var(--text-muted);">
      <span aria-hidden="true" style="font-size: 2em; display: block; margin-bottom: 0.5em;">🔍</span>
      <p style="margin: 0;">Enter a question above to start querying your knowledge graph.</p>
    </div>
  {/if}

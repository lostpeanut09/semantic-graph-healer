...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-empty-state" style="padding: 2em; text-align: center; border: 2px dashed var(--background-modifier-border); border-radius: 8px; color: var(--text-muted);">
      <div aria-hidden="true" style="font-size: 2em; margin-bottom: 0.5em;">🔍</div>
      <p>Ask a question above to start querying your knowledge graph.</p>
    </div>
  {/if}

...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-card healer-empty-state" style="padding: 2em; text-align: center; border-style: dashed; border-color: var(--background-modifier-border); color: var(--text-muted); border-radius: 8px; margin-top: 1em;">
      <span aria-hidden="true" style="font-size: 2em; display: block; margin-bottom: 0.5em;">🔍</span>
      <p style="margin: 0;">{hasSearched ? 'No relevant information found in the knowledge graph.' : 'Ask a question to explore your knowledge graph.'}</p>
    </div>
  {/if}

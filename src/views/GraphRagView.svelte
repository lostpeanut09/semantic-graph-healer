...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-empty-state" style="border: 2px dashed var(--background-modifier-border); padding: 2em; text-align: center; border-radius: 8px; color: var(--text-muted); margin-top: 1em;">
      <span aria-hidden="true" style="font-size: 2em; margin-bottom: 0.5em; display: block;">✨</span>
      <p style="margin: 0;">Enter a query above to start exploring your knowledge graph with GraphRAG.</p>
    </div>
  {/if}

...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-card healer-empty-state" style="padding: 2em; text-align: center; border-style: dashed; border-color: var(--background-modifier-border); border-width: 1px; border-radius: 8px; color: var(--text-muted); margin-top: 1em;">
      <span aria-hidden="true" style="font-size: 2em; display: block; margin-bottom: 0.5em;">🔍</span>
      <p style="margin: 0;">Ask a question to explore your vault's semantic clusters.</p>
    </div>
  {/if}

...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-card healer-empty-state" style="padding: 2em; text-align: center; border-style: dashed; border-color: var(--background-modifier-border); border-width: 2px;">
      <span aria-hidden="true" style="font-size: 2em; display: block; margin-bottom: 0.5em;">🔍</span>
      <p style="color: var(--text-muted); margin: 0;">Ask a global question about your vault to synthesize insights from your knowledge graph.</p>
    </div>
  {/if}

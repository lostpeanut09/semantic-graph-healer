...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer}
    <div class="healer-empty-state" style="padding: 2em; border: 2px dashed var(--background-modifier-border); border-radius: 8px; text-align: center; margin-top: 1em;">
      <span aria-hidden="true" style="font-size: 2em; color: var(--text-muted); display: block; margin-bottom: 0.5em;">🔍</span>
      <p style="color: var(--text-muted); margin: 0;">Ask a question to explore insights from your knowledge graph.</p>
    </div>
  {/if}

...
<div class="healer-graphrag-container" style="padding: 1em; display: flex; flex-direction: column; gap: 1em;">
  ...
  {#if !loading && !answer && communities.length === 0}
    <div class="healer-empty-state healer-card" style="border-style: dashed; text-align: center; padding: 2em; margin-top: 1em; color: var(--text-muted);">
      <div aria-hidden="true" style="font-size: 2em; margin-bottom: 0.5em;">🔍</div>
      <p style="margin: 0;">Search the knowledge graph to discover cross-community insights and structural patterns.</p>
    </div>
  {/if}

...
- **Note:** WorkerActionType is used in several files (types.ts, GraphWorkerService.ts, AutomationApi.ts, graph-analysis-core.ts). The plan's intention to delete it from types.ts is likely based on Knip's report of it being unused _at that specific export location_ or replaced by a more local definition.
